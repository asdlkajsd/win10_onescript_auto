...
set "line1============================================================="
set "line2=************************************************************"
set "line3=__________________________________________________"
...
if %AUR% EQU 0 (set "_dHook=Not Installed") else (set "_dHook=활성화 됨")
...
call :Cfgbg 07 "               [1] 인증하기 " 1F "[%_dMode% 모드]"
)
if %AUR% EQU 1 (           
call :Cfgbg 07 "               [2] 자동 인증 모드 설치 " 0A "[%_dHook%]"
) else (
echo                [2] 자동 인증 모드 설치
)
echo                [3] 인증 관련 프로세스 삭제
echo.
echo                %line4%
echo.
echo                [설정]:
echo.
if %_dAwin%==Yes (
echo                	  [4] Windows 인증             [ %_dAwin% ]
) else (
call :Cfgbg 07 "                  [4] Windows 인증             " %_cYel% "[ %_dAwin% ]"
)
if %_dAoff%==Yes (
echo                	  [5] Office 인증              [ %_dAoff% ]
) else (
call :Cfgbg 07 "                  [5] Office 인증              " %_cYel% "[ %_dAoff% ]"
)
echo                	  [6] 인증 상태 확인 [vbs]
echo                	  [7] 인증 상태 확인 [wmic]
if %winbuild% GEQ 10240 (
if %_dWXKMS%==Yes (
echo                	  [X] Windows 10 KMS38 생략    [ %_dWXKMS% ]
) else (
call :Cfgbg 07 "                  [X] Windows 10 KMS38 생략    " %_cYel% "[ %_dWXKMS% ]"
))
echo.
echo                %line4%
echo.
popd
choice /c 12345678x0 /n /m ">               메뉴를 선택하세요, 0을 입력하면 종료됩니다: "

set _el=%errorlevel%
if %_el%==10 (exit)
if %_el%==9 if %winbuild% GEQ 10240 (if %SkipKMS38% EQU 0 (set SkipKMS38=1) else (set SkipKMS38=0))&goto :MainMenu
if %_el%==8 goto :E_IP
...
set kip=
echo.
echo KMS 서버 주소를 입력하거나 붙여넣으세요, 아무것도 입력하지 않고 엔터를 누르면 이전으로 돌아갑니다.:
echo.
set /p kip=
...
set "nKMS=KMS 인증 미지원..."
set "nEval=평가판은 인증할 수 없습니다. Full Windows OS를 설치하세요."
if defined EditionID echo %EditionID%| findstr /I /E Eval %_Nul1% && (
set _eval=1
echo %EditionID%| findstr /I /B Server %_Nul1% && (set "nEval=서버 평가판은 인증할 수 없습니다. Full Server OS를 설치하세요.")
)
set "_C16R="
...
if "%_rtr%"=="DoActivate" (
echo.
echo ▼이 파일은 반드시 백신 프로그램에서 예외 설정돼야 합니다.
...
if %loc_offgl% EQU 0 (echo.&echo 설치된 Office 2013/2016/2019 제품 찾을 수 없음...&exit /b)
:: Retail C2R scenario
if %RunR2V% EQU 0 if %AutoR2V% EQU 1 goto :C2RR2V
:ReturnSPP
:: Retail MSI scenario or failed C2R-R2V scenario
echo.
if %loc_off15% EQU 1 if %vol_off15% EQU 0 echo Office 2013 발견 %nKMS%
if %loc_off16% EQU 1 if %vol_off16% EQU 0 echo Office 2016 발견 %nKMS%
if %loc_off19% EQU 1 if %vol_off19% EQU 0 echo Office 2019 발견 %nKMS%
echo Retail 제품은 먼저 Volume으로 전환해야합니다.
exit /b
...
if %WinPerm% EQU 1 (
for /f "tokens=2 delims==" %%x in ('"wmic path %spp% where (ApplicationID='%_wApp%' and LicenseStatus='1') get Name /VALUE"') do echo 확인중: %%x
echo 제품이 영구적으로 정품 인증되어있습니다.
exit /b
)
call :insKey
exit /b
...
if %OsppHook% EQU 0 (echo.&echo 설치된 Office %aword% 를 찾을 수 없음...&exit /b)
if %winbuild% GEQ 9200 if %loc_off14% EQU 0 (echo.&echo 설치된 Office %aword% 찾을 수 없음...&exit /b)
if %winbuild% GEQ 9200 wmic path %spp% get Description %_Nul2% | findstr /i KMSCLIENT %_Nul1% || (echo.&echo Office %aword% 발견 %nKMS%&echo Retail 제품은 먼저 Volume으로 전환해야합니다.&exit /b)
...
set Off1ce=0
:: nothing installed scenario
if %loc_offgl% EQU 0 (echo.&echo Office %aword% 찾을 수 없음...&exit /b)
:: Retail C2R scenario
if %RunR2V% EQU 0 if %AutoR2V% EQU 1 goto :C2RR2V
:ReturnOSPP
:: Retail MSI scenario or failed C2R-R2V scenario
echo.
if %loc_off14% EQU 1 if %vol_off14% EQU 0 echo Office 2010 발견 %nKMS%
if %loc_off15% EQU 1 if %vol_off15% EQU 0 echo Office 2013 발견 %nKMS%
if %loc_off16% EQU 1 if %vol_off16% EQU 0 echo Office 2016 발견 %nKMS%
if %loc_off19% EQU 1 if %vol_off19% EQU 0 echo Office 2019 발견 %nKMS%
echo Retail 제품은 먼저 Volume으로 전환해야합니다.
...
echo.
set "_key="
for /f "tokens=2 delims==" %%A in ('"wmic path %spp% where ID='%app%' get Name /VALUE"') do echo 제품 키 설치 : %%A
call :keys %app%
if "%_key%"=="" (echo 알맞은 KMS Client key를 찾을 수 없음&exit /b)
...
for /f "tokens=2 delims==" %%x in ('"wmic path %spp% where ID='%app%' get Name /VALUE"') do echo 확인중: %%x
echo 제품이 KMS 2038 인증되었습니다.
exit /b
)
for /f "tokens=2 delims==" %%x in ('"wmic path %spp% where ID='%app%' get Name /VALUE"') do echo 인증 중: %%x
...
echo 제품 인증 성공
echo 남은 기간: %gpr2% days ^(%gpr% minutes^)
exit /b
)
if %gpr% EQU 64800 (
echo 제품 인증 성공
